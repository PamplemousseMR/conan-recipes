...
class BoostConan(ConanFile):
    ...
    def requirements(self):
        if tools.os_info.is_windows:
            self.requires("zlib/1.2.11@{0}/{1}".format(self.user, self.channel))
    ...
    def get_build_flags(self):
        ...
        if self.zip_bzip2_requires_needed:
            add_defines(self.options.zlib and tools.os_info.is_windows, "zlib")
    ...
    def create_user_config_jam(self, folder):
        ...
        if self.zip_bzip2_requires_needed:
            ...
            contents = ""
            if self.options.zlib and tools.os_info.is_windows:
