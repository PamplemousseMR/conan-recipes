...
class GlewConan(ConanFile):
    ...
    def package_info(self):
        ...
        if self.options.shared:
            self.cpp_info.components["glew_s"].name = "glew"
        else:
            self.cpp_info.components["glew_s"].name = "glew_s"
        self.cpp_info.components["glew_s"].libs = tools.collect_libs(self)
        
        if not self.options.shared:
            self.cpp_info.components["glew_s"].defines.append("GLEW_USE_STATIC_LIBS")

        if tools.os_info.is_windows:
            if self.settings.compiler == "Visual Studio":
                if not self.options.shared:
                    self.cpp_info.components["glew_s"].libs.append("OpenGL32.lib")
                    if self.settings.compiler.runtime != "MT":
                        self.cpp_info.components["glew_s"].exelinkflags.append("-NODEFAULTLIB:LIBCMTD")
                        self.cpp_info.components["glew_s"].exelinkflags.append("-NODEFAULTLIB:LIBCMT")
            else:
                self.cpp_info.components["glew_s"].libs.append("opengl32")

        else:
            if tools.os_info.is_macos:
                self.cpp_info.components["glew_s"].exelinkflags.append("-framework OpenGL")
            elif not self.options.shared:
                self.cpp_info.components["glew_s"].libs.append("GL")
