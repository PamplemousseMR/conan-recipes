...
class OgreConan(ConanFile):
    ...
    license = "MIT"
    generators = "cmake"	# TODO Use cmake_find_package instead of cmake.
    ...
    default_options = {
        ...
        "plugin_octree": True,
        "plugin_bsp": True,
        "plugin_cg": False,         # TODO Cg_FOUND;NOT APPLE_IOS;NOT WINDOWS_STORE;NOT WINDOWS_PHONE
        "plugin_exrcodec": False,   # TODO doesn't work, not fond by Ogre's cmake.
        "plugin_stbi": True,
        "plugin_freeimage": False,  # TODO FreeImage_FOUND
        "plugin_pfx": True,
        "plugin_pcz": True,
        "plugin_dot_scene": True,
        "plugin_assimp": False,     # TODO assimp
        "rendersystem_d3d9": False,
        "rendersystem_d3d11": False,
    ...
    def requirements(self):
        if self.options.component_overlay:
            self.requires("freetype/2.10.4@{0}/{1}".format(self.user, self.channel))
        if self.options.config_enable_zzip:
            self.requires("zzip/0.13.71@{0}/{1}".format(self.user, self.channel))
        if self.options.plugin_dot_scene:
            self.requires("pugixml/1.10@{0}/{1}".format(self.user, self.channel))
        if self.options.plugin_exrcodec:
            self.requires("openexr/2.5.3@{0}/{1}".format(self.user, self.channel))

    def source(self):
        tools.get("{0}/archive/v{1}.tar.gz".format(self.homepage, self.version),
                  sha256="028e7d0bbfce8fd254e1102666b07b7cbb9379321e88885d82feee5e061342aa")
        os.rename("{0}-{1}".format(self.name, self.version), self._source_folder)
        os.rename(os.path.join(self._source_folder, "CMakeLists.txt"),
                  os.path.join(self._source_folder, "WrappedCMakeLists.txt"))
        shutil.copy(self.exports_sources, self._source_folder)

    def build(self):
        cmake = CMake(self)
        cmake.definitions["OGRE_CONAN_INFO_DIR"] = self.build_folder
    ...
    def package_info(self):
        ...
        if self.options.component_hlms:
            if not any("OgreHLMS" in s for s in self.cpp_info.libs):
                self.output.warn("HLMS component as not been built.")
        ...
        if self.options.plugin_assimp:
            if not any("Plugin_Assimp" in s for s in self.cpp_info.libs):
        ...
        if self.options.rendersystem_gles2:
            if not any("rendersystem_gles2" in s for s in self.cpp_info.libs):
                self.output.warn("RenderSystem_GLES2 RenderSystem as not been built.")
